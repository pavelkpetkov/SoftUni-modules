...
const endpoints = {
    ...
    login: '/users/login',
    logout: '/users/logout',
    catalog: '/data/pets?sortBy=_createdOn%20desc',
    create: '/data/pets',
    like: '/data/likes',
    details: (id) => `/data/pets/${id}`,
    profile: (id) => `/data/pets?where=_ownerId%3D%22${id}%22&sortBy=_createdOn%20desc`,
    total: (petId) => `/data/likes?where=petId%3D%22${petId}%22&distinct=_ownerId&count`,
    own: (petId, userId) => `/data/likes?where=petId%3D%22${petId}%22%20and%20_ownerId%3D%22${userId}%22&count`
};
...
describe('E2E tests', function () {
    ...
    describe('Navigation bar [ 5 Points ]', () => {
        it('guest user should see correct navigation [ 2 Points ]', async () => {
            ...
            expect(await page.isVisible('nav >> text=My Pets')).to.be.false;
            expect(await page.isVisible('nav >> text=Add Pet')).to.be.false;
        ...
        it('logged user should see correct navigation [ 2 Points ]', async () => {
            ...
            expect(await page.isVisible('nav >> text=My Pets')).to.be.true;
            expect(await page.isVisible('nav >> text=Add Pet')).to.be.true;
    ...
    describe('Catalog [ 20 Points ]', () => {
        it('show empty catalog [ 2.5 Points ]', async () => {
            ...
            expect(await page.isVisible('text=No pets in database!')).to.be.true;
        });
        ...
        it('show catalog [ 7.5 Points ]', async () => {
            ...
            expect(titles[2]).to.contains(mockData.catalog[2].name);
            expect(titles[3]).to.contains(mockData.catalog[3].name);
            expect(titles[4]).to.contains(mockData.catalog[4].name);
            expect(await page.isVisible('text=No pets in database!')).to.be.false;
        });
        ...
        it('show details [ 5 Points ]', async () => {
            ...
            await page.click(`#dashboard-page > ul li:has-text("${data.name}") >> text=Details`);
            await page.waitForTimeout(interval);

            expect(await page.isVisible(`h3:has-text("${data.name}")`)).to.be.true;
            expect(await page.isVisible(`.type:has-text("${data.type}")`)).to.be.true;
            expect(await page.isVisible(`.pet-description > p:has-text("${data.description}")`)).to.be.true;
    ...
    describe('CRUD [ 45 Points ]', () => {
        ...
        it('edit should populate form with correct data [ 5 Points ]', async () => {
            ...
            const formData = {
                name: await page.$eval('[name="name"]', t => t.value),
            ...
            expect(formData.name).to.equal(data.name);
    ...
    describe('My Pets Page [ 10 Points ]', async () => {
        // Login user
        beforeEach(async () => {
            const data = mockData.users[0];
            await page.goto(host);
        ...
        it('the logged-in user should be able to see his/her own pets (test with 2 pets) [ 2 Points ]', async () => {
            await page.goto(host);
            await page.waitForTimeout(interval);
            await page.click('text=My Pets');
            await page.waitForTimeout(interval);

            const titles = await page.$$eval('#my-pets-page ul li', t => t.map(s => s.textContent));
            expect(titles.length).to.equal(2);
            expect(await page.isVisible('text=No pets in database!')).to.be.false;
        });

        it('the logged-in user should be able to see "No pets in database!" (test with 0 pets) [ 2 Points ]', async () => {
        ...
        it('the logged-in user should be able to view his/her own pets - detailed information [ 4 Points ]', async () => {
            ...
            const titles = await page.$$eval('#my-pets-page ul li', t => t.map(s => s.textContent));
            const firstImg = await page.getAttribute(`.my-pets-list > li:has-text("${mock[0].name}") img`, 'src');
            const secodnImg = await page.getAttribute(`.my-pets-list > li:has-text("${mock[1].name}") img`, 'src');
            const thirdImg = await page.getAttribute(`.my-pets-list > li:has-text("${mock[2].name}") img`, 'src');
            await page.waitForTimeout(interval);

            expect(titles.length).to.equal(3);
            expect(titles[0]).to.contains(mock[0].name);
            expect(titles[0]).to.contains(mock[0].type);
            expect(titles[1]).to.contains(mock[1].name);
            expect(titles[1]).to.contains(mock[1].type);
            expect(titles[2]).to.contains(mock[2].name);
        ...
        it('the logged-in user should be redirected to the Details page after click on the button "Details" [ 2 Points ]', async () => {
            ...
            await page.click(`.my-pets-list > li:has-text("${data.name}") >> text=Details`);
            await page.waitForTimeout(interval);
            await page.waitForSelector('.pet-information');
    ...
    describe('BONUS : Like functionality  [ 10 Points ]', async () => {
        ...
        it('Like button should be hidden(not visible) after а click on it [ 2 Points ]', async () => {
            ...
            await page.waitForTimeout(interval);
            expect(postData.petId).to.equal(mockData.likes[2].petId);
